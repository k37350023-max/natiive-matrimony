  ...
  return (
    <section style={{ maxWidth: '720px', margin: '0 auto 80px', padding: '0 20px' }}>
      <div style={{ textAlign: 'center', marginBottom: '36px' }}>
        <p style={{ fontSize: '11.5px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#7F1D1D', margin: '0 0 10px' }}>Common questions</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {FAQS.map((faq, i) => (
          <div key={i} style={{ background: 'white', borderRadius: '14px', border: `1px solid ${open === i ? '#FECACA' : 'rgba(0,0,0,0.06)'}`, overflow: 'hidden', transition: 'border-color 0.2s', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
            <button
              ...
              <span style={{ fontSize: '14.5px', fontWeight: 600, color: open === i ? '#7F1D1D' : '#1F2937', lineHeight: 1.4 }}>{faq.q}</span>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={open === i ? '#7F1D1D' : '#9CA3AF'} strokeWidth="2.5" style={{ flexShrink: 0, transform: open === i ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
                <polyline points="6 9 12 15 18 9"/>
              </svg>
            </button>
            {open === i && (
              <div style={{ padding: '0 20px 18px' }}>
                <p style={{ fontSize: '13.5px', color: '#6B7280', lineHeight: 1.7, margin: 0 }}>{faq.a}</p>
              </div>
            )}
          </div>
        ))}
