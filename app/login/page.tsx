  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (data.needsProfile) {
        // New user — carry the verified Google identity into the quick signup.
        sessionStorage.setItem('nm_google', JSON.stringify({ idToken, email, name }))
      ...
      <div style={{ flex: 1, display: 'flex' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: '34px 16px 40px' }}>
          <div style={{ width: '100%', maxWidth: '360px' }}>
            ...
            {/* Google sign-in */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '18px 2px 14px' }}>
              <div style={{ flex: 1, height: '1px', background: '#E7E3D8' }} />
              <span style={{ fontSize: '12px', color: '#94A3B8', fontWeight: 600 }}>or</span>
              <div style={{ flex: 1, height: '1px', background: '#E7E3D8' }} />
            </div>
            <button onClick={handleGoogle} disabled={loading}
              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '12px', fontSize: '14.5px', fontWeight: 700, color: '#14241C', background: 'white', border: '1.5px solid #E7E3D8', borderRadius: '10px', cursor: 'pointer' }}>
