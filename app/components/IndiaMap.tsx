  ...
  return (
    <>
      ...
      <div style={{
        ...
        margin: '0 auto', background: '#FBFAF5',
      }}>
        <svg
        ...
        >
          {IndiaData.locations.map(loc => {
            ...
            let sw = '0.5'
            if (isTelugu) {
              fill = isOn ? '#EAF3EA' : '#D6C9AA'
              stroke = isOn ? '#14241C' : '#9E8E6A'
              sw = isOn ? '2.5' : '1'
            }
            return (
          ...
          <text x={TG_CX} y={TG_CY} textAnchor="middle" fontSize={compact ? '7' : '9'} fontWeight="700"
            fill={highlightedId === 'tg' ? '#14241C' : '#7A6F5A'}
          ...
          <text x={AP_CX} y={AP_CY} textAnchor="middle" fontSize={compact ? '6' : '8'} fontWeight="700"
            fill={highlightedId === 'ap' ? '#14241C' : '#7A6F5A'}
            style={{ pointerEvents: 'none', userSelect: 'none', transition: 'fill .3s' }}>
            {apLabel}
          </text>
          {mode === 'filter' && highlightedId === 'ap' && (
            <text x={AP_CX} y={AP_CY + 10} textAnchor="middle" fontSize="5" fill="#14241C"
          ...
          {mode === 'animated' && animHighlight && (() => {
            ...
            return (
              <circle cx={cx} cy={cy} r="12"
                fill="none" stroke="#14241C" strokeWidth="2.5"
