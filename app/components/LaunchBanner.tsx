  ...
  if (isPremium && premiumExpiry) {
    ...
    return (
      <div className="mx-4 my-3 rounded-xl px-4 py-2.5 flex items-center justify-between"
        style={{ background: '#EAF3EA', border: '1px solid #A7F3D0' }}>
    ...
    <Link href={isLoggedIn ? '/pricing' : '/register'}
      ...
      <div className="px-5 py-5 flex items-center gap-5">
        ...
        <div className="shrink-0 w-10 h-10 rounded-xl flex items-center justify-center"
          ...
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#14241C" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round">
          ...
          {count !== null && (
            <div className="mt-2 flex items-center gap-2">
              <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(28,25,23,0.18)' }}>
                <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: '#14241C' }} />
              </div>
              <span className="text-[10px] font-bold shrink-0" style={{ color: '#14241C' }}>{remaining.toLocaleString()} left</span>
            </div>
          )}
        </div>
