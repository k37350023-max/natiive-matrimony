...
export default function FounderTracker() {
  ...
  return (
    <div className="mx-4 sm:mx-auto sm:max-w-lg rounded-2xl overflow-hidden border bg-white"
      style={{ borderColor: '#E7E3D8', boxShadow: '0 4px 24px rgba(155,28,28,0.10)' }}>

      {/* Header */}
      <div className="px-6 pt-6 pb-5" style={{ background: 'linear-gradient(135deg, #14241C 0%, #14241C 100%)' }}>
        <p className="text-xs font-bold uppercase tracking-widest text-red-300 mb-1">Always Free + Premium Trial</p>
        <p className="text-white font-bold text-xl leading-snug font-serif-display">
          Free version stays free.<br />
          <span className="text-red-200">Premium is free for founders, then 3 months for others.</span>
        </p>
      </div>
      ...
      <div className="px-6 pt-5 pb-4">
        <div className="flex items-end justify-between mb-2">
          ...
          <p className="text-sm font-bold" style={{ color: '#14241C' }}>{pct}% full</p>
        </div>

        <div className="h-2.5 rounded-full overflow-hidden bg-gray-100">
          <div className="h-full rounded-full transition-all duration-700"
            style={{ width: `${Math.max(pct, 2)}%`, background: 'linear-gradient(90deg, #14241C, #DC2626)' }} />
        </div>
          ...
          {PERKS.map(perk => (
            <div key={perk} className="flex items-center gap-2">
              <span className="w-4 h-4 rounded-full flex items-center justify-center shrink-0"
                ...
                <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#14241C" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round">
