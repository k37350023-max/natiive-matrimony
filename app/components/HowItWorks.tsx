  ...
  return (
    <section style={{ maxWidth: '900px', margin: '0 auto 80px', padding: '0 20px' }}>
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        <p style={{ fontSize: '11.5px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#14241C', margin: '0 0 10px' }}>Simple process</p>
          ...
          <div key={s.n} style={{ position: 'relative', background: 'white', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.06)', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            ...
            {i < steps.length - 1 && (
              <div style={{
                ...
                background: '#CADFCA', zIndex: 1,
              }} className="hiw-connector" />
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: '#EDF3ED', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {s.icon}
              </div>
              <span style={{ fontSize: '11px', fontWeight: 800, color: '#CADFCA', letterSpacing: '0.04em' }}>STEP {s.n}</span>
            </div>
            <p style={{ fontSize: '15px', fontWeight: 700, color: '#0F0F0F', margin: '0 0 8px', letterSpacing: '-0.01em' }}>{s.title}</p>
            <p style={{ fontSize: '13px', color: '#5E6B62', lineHeight: 1.6, margin: 0 }}>{s.desc}</p>
          </div>
        ))}
      </div>
    </section>
