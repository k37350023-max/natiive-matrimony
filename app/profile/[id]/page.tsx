...
export default function ProfilePage() {
  ...
  async function loadProfile(myId: string | null) {
    // Secured read: phone/email redacted for non-owners (identity from session cookie).
    const data = await fetch(`/api/profiles/${id}`).then(r => r.ok ? r.json() : { profile: null }).then(j => j.profile)
    setProfile(data)
    ...
    <div className="min-h-screen pb-28" style={{ background: '#FBFAF5' }}>
      ...
      <div className="max-w-3xl mx-auto px-4 py-5 space-y-3">
        ...
        {isOwnProfile && (() => {
          ...
          const missing = missingFields.map(m => m.label).slice(0, 3)
          return (
            <div className="card px-5 py-4">
        ...
        {!canViewFullProfile && !isOwnProfile && (
          <div className="card px-5 py-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-800">This member chose to hide some profile details.</p>
                <p className="text-xs text-gray-500 mt-1 leading-relaxed">Connect first to see the hidden details.</p>
              </div>
            </div>
          </div>
        )}
          ...
          return (
            <div className="card px-6 py-5">
              ...
              {!showContact && !isOwnProfile && (
                <p className="text-xs text-gray-400 mt-2">Contact details are shown after connection.</p>
              )}
            </div>
          )
        })()}
              ...
              {!interestSent && fullProfileHidden && (
                <div className="flex justify-center mt-2">
                  <span className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1 rounded-full"
                    ...
                    Some details are hidden until connection
                  </span>
                </div>
              )}
            </>
